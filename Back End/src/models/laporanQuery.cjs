...
const getNewLaporan = () => {
  const query = "SELECT COUNT(*) as count FROM laporan_lingkungan WHERE status = 'Aktif'";
  try {
    return dbPool.query(query);
  } catch (err) {
    throw err;
...
const getLaporanActive = () => {
  const query = "SELECT id_laporan, nama, tanggal_laporan, lokasi_laporan, deskripsi_laporan, status FROM laporan_lingkungan WHERE status = 'Aktif' ORDER BY tanggal_laporan DESC";
  try {
    return dbPool.execute(query); // Changed to execute for consistent return format
  } catch (err) {
